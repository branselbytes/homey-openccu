...
export function parseStoredBindings(
  ...
  if (!Array.isArray(value))
    throw new TypeError("Stored OpenCCU bindings must be an array");
...
function parseBinding(value: unknown, index: number): CapabilityBinding {
  if (!isRecord(value))
    throw new TypeError(`Stored binding ${index} must be an object`);
  const capability = requiredString(value.capability, index, "capability");
  const channelAddress = requiredString(
  ...
  if (
    ...
    throw new TypeError(
      `Stored binding ${index} must define readable and writable booleans`,
    );
  }
  if (
  ...
  const writeChannelAddress = optionalString(
    value.writeChannelAddress,
    index,
    "writeChannelAddress",
  );
  const writeParameter = optionalString(
    value.writeParameter,
    index,
    "writeParameter",
  );
  const writeStrategy = value.writeStrategy ?? "direct";
  if (
    typeof writeStrategy !== "string" ||
    !WRITE_STRATEGIES.includes(writeStrategy as WriteStrategy)
  ) {
    throw new TypeError(
      `Stored binding ${index} has an unsupported write strategy`,
    );
  }
  ...
  return {
    ...
    ...(writeStrategy === "direct"
      ? {}
      : { writeStrategy: writeStrategy as WriteStrategy }),
  };
}
